...
public class RestaurantService {
	...
	public List<PositionDto> getPosition(List<Restaurant> data) {
		...
		for (int i = 0; i < data.size(); i++) {
			...
			positionDto.setMapid(data.get(i).getMapId());
			positionDto.setX(data.get(i).getX());
			positionDto.setY(data.get(i).getY());

			String content = String.format("<div style=\"width:180px; background:#dddddd; text-align:center;\"><span style=\"font-weight:bold\">"
				+ "%s</span><br>"
